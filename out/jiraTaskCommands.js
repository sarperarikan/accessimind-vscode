...
exports.initializeJiraTaskCommands = initializeJiraTaskCommands;
exports.createJiraTask = createJiraTask;
...
function getWcagCriteriaInfo(criteriaNumber) {
...
function initializeJiraTaskCommands(aiProvider, statsManager) {
...
async function createJiraTask() {
    ...
    try {
        ...
        const customPrompt = config.get("jira.customPrompt") || "";
        const includeCustomPrompt = config.get("jira.useCustomPrompt") || false;
        ...
        await vscode.window.withProgress({
            ...
            const jiraTaskContent = await generateJiraTaskContent(provider, codeToAnalyze, language, fileName, taskDetails);
            ...
            if (jiraTaskContent.success) {
                ...
                vscode.window.showInformationMessage(`✅ Jira task created successfully! Found ${jiraTaskContent.wcagCriteria?.length || 0} WCAG criteria to address.`, "Copy to Clipboard", "Save to File").then(action => {
                ...
                vscode.window.showErrorMessage(`❌ Jira task creation failed: ${jiraTaskContent.error || "Unknown error"}`);
            }
        });
    }
    catch (error) {
        ...
        logger_1.logger.error("Jira task creation error:", error);
    }
}
async function getJiraTaskDetails(customPrompt, includeCustomPrompt) {
...
async function generateJiraTaskContent(provider, code, language, fileName, taskDetails) {
    const basePrompt = `
You are a WCAG accessibility expert. Analyze the provided code and create a comprehensive Jira ${taskDetails.issueType} for accessibility improvements.

**Code Analysis Context:**
- File: ${fileName}
- Language: ${language}
- Issue Type: ${taskDetails.issueType}
...
function extractWcagCriteriaFromJiraContent(content) {
    const wcagPattern = /(\d+\.\d+\.\d+)/g;
    const matches = content.match(wcagPattern) || [];
    return [...new Set(matches)]; // Remove duplicates
}
async function showJiraTaskContent(content, fileName, wcagCriteria, issueType) {
    const panel = vscode.window.createWebviewPanel("accessimindJiraTask", "🎫 Jira Task - WCAG Analysis", vscode.ViewColumn.Beside, {
        enableScripts: true,
...
async function exportJiraJson(jiraJson, fileName) {
...
async function saveJiraTaskToFile(content, fileName) {
...
function getJiraTaskWebviewContent(content, fileName, jiraJson, wcagCriteria, summary, description) {
...
//# sourceMappingURL=jiraTaskCommands.js.map