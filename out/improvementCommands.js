...
exports.initializeImprovementCommands = initializeImprovementCommands;
exports.improveCurrentFile = improveCurrentFile;
exports.improveSelectedCode = improveSelectedCode;
exports.improveCurrentSelected = improveCurrentSelected;
...
function initializeImprovementCommands(aiProvider, statsManager, localization, statsProvider, modernStatsProvider, statusBarCallback) {
    ...
    try {
        await vscode.window.withProgress({
            ...
            if (improvementResult.success && improvementResult.improvedCode) {
                ...
                vscode.window.showErrorMessage(`❌ İyileştirme başarısız: ${improvementResult.error || "Bilinmeyen hata"}`);
            }
        });
    }
    catch (error) {
        ...
        logger_1.logger.error("Dosya iyileştirme hatası:", error);
    }
}
    ...
    try {
        await vscode.window.withProgress({
            ...
            if (improvementResult.success && improvementResult.improvedCode) {
                ...
                vscode.window.showErrorMessage(`❌ Seçili kod iyileştirmesi başarısız: ${improvementResult.error || "Bilinmeyen hata"}`);
            }
        });
    }
    catch (error) {
        ...
        logger_1.logger.error("Seçili kod iyileştirme hatası:", error);
    }
}
async function improveCurrentSelected() {
    const editor = vscode.window.activeTextEditor;
    if (!editor) {
        vscode.window.showErrorMessage("❌ Aktif bir dosya bulunamadı");
        return;
...
//# sourceMappingURL=improvementCommands.js.map