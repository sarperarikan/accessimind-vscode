...
const statsViewProvider_1 = require("./views/statsViewProvider");
const modernStatsViewProvider_1 = require("./views/modernStatsViewProvider");
const settingsViewProvider_1 = require("./views/settingsViewProvider");
const tabbedMainViewProvider_1 = require("./views/tabbedMainViewProvider");
const wcagImprover_1 = require("./core/wcagImprover");
...
const persistentSettingsManager_1 = require("./utils/persistentSettingsManager");
const accessiMindJsonManager_1 = require("./utils/accessiMindJsonManager");
const settingsManager_1 = require("./utils/settingsManager");
const logger_1 = require("./utils/logger");
const improvementCommands_1 = require("./improvementCommands");
const providerCommands_1 = require("./providerCommands");
const jiraTaskCommands_1 = require("./jiraTaskCommands");
const wizardManager_1 = require("./wizardManager");
let wcagImprover;
let aiProviderManager;
let statisticsManager;
let statsViewProvider;
let modernStatsViewProvider;
let settingsViewProvider;
let tabbedMainViewProvider;
let localization;
let persistentSettingsManager;
let jsonManager;
let settingsManager;
let statusBarItem;
let wizardManager;
async function activate(context) {
    logger_1.logger.info('🚀 AccessiMind activation starting...');
    // Initialize core components
    wcagImprover = new wcagImprover_1.WcagImprover();
    aiProviderManager = aiProvider_1.AIProviderManager.getInstance();
    statisticsManager = statisticsManager_1.StatisticsManager.getInstance(context);
    localization = localizationManager_1.LocalizationManager.getInstance();
    wizardManager = wizardManager_1.WizardManager.getInstance();
    // Initialize persistent settings manager
    persistentSettingsManager = persistentSettingsManager_1.PersistentSettingsManager.getInstance(context);
    await persistentSettingsManager.initialize();
    // Initialize JSON manager
    jsonManager = accessiMindJsonManager_1.AccessiMindJsonManager.getInstance(context);
    ...
    statsViewProvider = new statsViewProvider_1.StatsViewProvider(context.extensionUri);
    modernStatsViewProvider = new modernStatsViewProvider_1.ModernStatsViewProvider(context.extensionUri);
    settingsViewProvider = new settingsViewProvider_1.SettingsViewProvider(context);
    tabbedMainViewProvider = new tabbedMainViewProvider_1.TabbedMainViewProvider(context.extensionUri, context);
    ...
    (0, improvementCommands_1.initializeImprovementCommands)(aiProviderManager, statisticsManager, localization, statsViewProvider, modernStatsViewProvider, updateStatusBar);
    // Initialize provider commands module
    (0, providerCommands_1.initializeProviderCommands)(aiProviderManager, statisticsManager, localization, statsViewProvider, modernStatsViewProvider, updateStatusBar);
    // Initialize Jira task commands module
    (0, jiraTaskCommands_1.initializeJiraTaskCommands)(aiProviderManager, statisticsManager);
    // Register view providers
    context.subscriptions.push(vscode.window.registerWebviewViewProvider('wcagEnhancer.statsView', statsViewProvider), vscode.window.registerWebviewViewProvider('wcagEnhancer.modernStatsView', modernStatsViewProvider), vscode.window.registerWebviewViewProvider('wcagEnhancer.tabbedMainView', tabbedMainViewProvider), vscode.window.registerTreeDataProvider('wcagEnhancer.settingsView', settingsViewProvider));
    // Register commands
    ...
    logger_1.logger.info('✅ AccessiMind successfully activated!');
}
function registerCommands(context) {
    // New WCAG analysis commands
    context.subscriptions.push(vscode.commands.registerCommand('wcagEnhancer.analyzeOpenCode', async () => {
        ...
        await (0, jiraTaskCommands_1.createJiraTask)();
    ...
    vscode.commands.registerCommand('wcagEnhancer.restoreSettings', async () => {
        ...
        const action = await vscode.window.showWarningMessage('⚠️ Are you sure you want to clear all persistent settings? This action cannot be undone.', { modal: true }, 'Clear Persistent Settings');
...
async function analyzeOpenCodeStructures() {
...
`;
            ...
            if (improvementResult.success && improvementResult.content) {
                ...
                updateStatusBar();
                vscode.window.showInformationMessage(`✅ WCAG analysis completed with ${currentProviderName}! ${linesImproved} lines improved (${processingTime}ms)`);
            }
            else {
                ...
                if (errorMessage.includes("API anahtarı") || errorMessage.includes("API key")) {
                    const action = await vscode.window.showErrorMessage("API anahtarı yapılandırması gerekli. Ayarlar sayfasına gitmek ister misiniz?", "Ayarlara Git");
...
async function analyzeSelectedCodeStructure() {
    const editor = vscode.window.activeTextEditor;
    if (!editor) {
        vscode.window.showErrorMessage("❌ No active file found");
        return;
...
`;
            ...
            if (improvementResult.success && improvementResult.content) {
                ...
                modernStatsViewProvider.updateStatistics(stats);
                updateStatusBar();
                vscode.window.showInformationMessage(`✅ Selected code WCAG analysis completed with ${currentProviderName}! ${linesImproved} lines improved (${processingTime}ms)`);
            }
            else {
                const errorMessage = improvementResult.error || "Bilinmeyen hata oluştu";
        ...
        logger_1.logger.error("Selected code WCAG analysis error:", error);
        let errorMessage = "Bilinmeyen hata oluştu";
        if (error instanceof Error) {
            errorMessage = error.message;
        }
...
async function showDetailedStatistics() {
    const stats = statisticsManager.getDetailedStatistics();
    const panel = vscode.window.createWebviewPanel('wcagStats', '📊 AccessiMind Statistics', vscode.ViewColumn.One, {
        enableScripts: true,
...
async function exportStatisticsCSV() {
    try {
        ...
        const csvData = [
            ...
            ["Today Improvements", stats.daily?.improvements || 0, "Improvements made today"],
            ["Today Lines", stats.daily?.linesImproved || 0, "Lines improved today"],
            // Monthly stats (safely access nested properties)
            ["This Month Improvements", stats.monthly?.improvements || 0, "Improvements this month"],
            ["This Month Lines", stats.monthly?.linesImproved || 0, "Lines improved this month"],
            // Most used language (safely access)
            ["Most Used Language", stats.mostUsedLanguage || "N/A", "Primary programming language"],
            // WCAG criteria (safely access)
            ...Object.entries(stats.wcagCriteriaStats || {}).map(([criteria, count]) => [`WCAG ${criteria}`, count, `Times this criteria was applied`])
        ];
...
async function resetStatistics() {
    const action = await vscode.window.showWarningMessage('⚠️ Are you sure you want to reset all statistics? This action cannot be undone.', { modal: true }, 'Reset Statistics');
...
function initializeStatusBar(context) {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('wcagEnhancer.showStatusBarMenu', async () => {
...
async function handleStatusBarMenuAction(action) {
    switch (action) {
...
async function showModernStatsPanel() {
    // Open modern statistics panel
    const panel = vscode.window.createWebviewPanel('wcagModernStats', '📊 AccessiMind - Modern Statistics', vscode.ViewColumn.One, {
        enableScripts: true,
        retainContextWhenHidden: true,
...
async function showPeriodStatistics(period) {
    const stats = statisticsManager.getDetailedStatistics();
    let periodStats;
    let title;
    let description;
    ...
    const panel = vscode.window.createWebviewPanel(`wcag${period.charAt(0).toUpperCase() + period.slice(1)}Stats`, title, vscode.ViewColumn.One, {
        enableScripts: true,
        retainContextWhenHidden: true
    });
...
async function showResetStatsMenu() {
    const resetOptions = [
        {
...
async function handleResetAction(action) {
    let confirmMessage;
    let buttonText;
    switch (action) {
        case "resetDaily":
...
async function resetMonthlyStatistics() {
    try {
...
async function resetYearlyStatistics() {
    try {
...
async function resetPeriodStatistics(period) {
    switch (period) {
        case "daily":
            await resetDailyStatistics();
            break;
...
async function showExportStatsMenu() {
    const exportOptions = [
        {
...
async function exportPeriodStatistics(period) {
    try {
        let data;
        let filename;
        switch (period) {
...
async function showWcagCommands() {
    const commands = [
        {
...
async function showSettingsInterface() {
    // Create modern status bar panel
    const panel = vscode.window.createWebviewPanel("wcagEnhancerStatusPanel", "♿ AccessiMind Dashboard", vscode.ViewColumn.One, {
        enableScripts: true,
        retainContextWhenHidden: true,
        localResourceRoots: [vscode.Uri.joinPath(vscode.workspace.workspaceFolders?.[0]?.uri || vscode.Uri.file(""), "media")]
    });
...
function registerDynamicKeybindings(context) {
    ...
    Object.entries(finalKeybindings).forEach(([command, key]) => {
        if (key && typeof key === 'string') {
            context.subscriptions.push(vscode.commands.registerCommand(`wcagEnhancer.keybinding.${command}`, async () => {
...
async function showWelcomeMessage(context) {
    ...
    if (!wizardCompleted || isPartiallyConfigured) {
        ...
        setTimeout(async () => {
            ...
            const action = await vscode.window.showInformationMessage(message, buttonText, 'Later');
...
async function showDetailedWelcomeScreen() {
...
function getPeriodStatsContent(periodStats, title, description, period) {
    return `
<!DOCTYPE html>
<html lang="tr">
<head>
...
function getStatsWebViewContent(stats) {
    return `
<!DOCTYPE html>
<html lang="en">
<head>
...
function getModernStatusPanelContent(stats) {
    ...
    const totalImprovements = getSafeValue(stats.totalImprovements, 0);
    const totalLines = getSafeValue(stats.totalLinesImproved, 0);
    const totalTokens = getSafeValue(stats.totalTokensUsed, 0);
    const avgTime = getSafeValue(stats.averageProcessingTime, 0);
    ...
    const dailyStats = stats.dailyStats?.[today] || { improvements: 0, linesImproved: 0 };
    const weeklyStats = stats.weeklyStats?.[weekStart] || { improvements: 0, linesImproved: 0 };
    const monthlyStats = stats.monthlyStats?.[thisMonth] || { improvements: 0, linesImproved: 0, tokensUsed: 0 };
    const yearlyStats = stats.yearlyStats?.[thisYear] || { improvements: 0, linesImproved: 0, tokensUsed: 0 };
    return `
<!DOCTYPE html>
<html lang="en">
<head>
			...
			<section class="panel" role="region" aria-labelledby="statistics-title">
				...
				<div style="margin-top: 24px;" role="region" aria-labelledby="wcag-criteria-title">
					...
					<table class="stats-table" role="table" aria-labelledby="wcag-criteria-title">
						...
						<tbody role="rowgroup">
							${Object.entries(stats.wcagCriteriaStats || {}).length > 0
        ? Object.entries(stats.wcagCriteriaStats || {}).map(([criteria, count]) => `
									<tr role="row">
...
async function loadSavedSettings() {
    try {
        ...
        if (aiConfig.provider) {
            ...
            logger_1.logger.info(`🔄 Saved AI provider loaded: ${aiConfig.provider}`);
        }
        ...
        if (selectedModel) {
            ...
            logger_1.logger.info(`🔄 Saved AI model loaded: ${selectedModel}`);
        }
        // API key'i yükle (güvenlik için log'lanmaz)
        if (aiConfig.apiKey) {
            logger_1.logger.info('🔄 Saved API key loaded');
        }
        // Dil ayarını yükle
        ...
        if (savedLanguage && savedLanguage !== 'auto' && (savedLanguage === 'en' || savedLanguage === 'tr')) {
            ...
            logger_1.logger.info(`🔄 Saved language loaded: ${savedLanguage}`);
        }
        // WCAG seviyesini yükle
        const wcagLevel = config.get('wcagLevel');
        if (wcagLevel) {
            logger_1.logger.info(`🔄 Saved WCAG level loaded: ${wcagLevel}`);
        }
        // Wizard tamamlanma durumunu kontrol et
        const wizardCompleted = config.get('wizardCompleted', false);
        if (wizardCompleted) {
            logger_1.logger.info('✅ AccessiMind fully configured from saved settings');
            ...
            if (aiConfig.selectedModel && !aiModelConfig.selectedModel) {
                ...
                logger_1.logger.info('🔧 Model setting synchronized to aiModels config');
            }
                ...
                logger_1.logger.info('🔧 Model setting synchronized to ai config');
            }
        }
        else {
            logger_1.logger.info('⚠️ AccessiMind configuration incomplete - wizard needed');
        }
    }
    catch (error) {
        logger_1.logger.error('❌ Error loading saved settings:', error);
    }
}
/**
...
async function loadSettingsFromJson() {
    try {
        if (!jsonManager) {
            logger_1.logger.warn("⚠️ JSON Manager henüz başlatılmamış");
            return;
        }
        ...
        if (settings.wizard.completed) {
            ...
            logger_1.logger.info("✅ JSON dosyasından ayarlar VS Code'a uygulandı");
        }
        else {
            logger_1.logger.info("ℹ️ Wizard henüz tamamlanmamış, JSON ayarları uygulanmadı");
        }
    }
    catch (error) {
        logger_1.logger.error("❌ JSON dosyasından ayar yükleme hatası:", error);
        // Hata durumunda kullanıcıyı bilgilendir
        vscode.window.showWarningMessage("⚠️ AccessiMind settings file could not be read. New settings will be created.");
    }
}
/**
        ...
        if (event.affectsConfiguration("wcagEnhancer") && jsonManager) {
            try {
                ...
                setTimeout(async () => {
                    ...
                    logger_1.logger.info("🔄 VS Code ayarları JSON dosyasına senkronize edildi");
                }, 500);
            }
            catch (error) {
                logger_1.logger.error("❌ VS Code ayarlarını JSON'a senkronize etme hatası:", error);
            }
        }
    });
    // Extension context'e listener'ı ekle (dispose için)
    if (jsonManager) {
        const context = jsonManager.context;
 ...
 */
function registerJsonManagerCommands(context) {
    // JSON ayarlarını VS Code'a uygula komutu
    context.subscriptions.push(vscode.commands.registerCommand('wcagEnhancer.applyJsonSettings', async () => {
        try {
            if (!jsonManager) {
            ...
            logger_1.logger.error("❌ JSON ayarlarını uygulama hatası:", error);
            vscode.window.showErrorMessage("❌ JSON ayarları uygulanamadı: " + error);
        }
    }));
    // VS Code ayarlarını JSON'a senkronize et komutu
    context.subscriptions.push(vscode.commands.registerCommand('wcagEnhancer.syncToJson', async () => {
        try {
            if (!jsonManager) {
                vscode.window.showErrorMessage("❌ JSON Manager bulunamadı");
                return;
            ...
            logger_1.logger.error("❌ JSON senkronizasyon hatası:", error);
            vscode.window.showErrorMessage("❌ Senkronizasyon başarısız: " + error);
        }
    }));
    // JSON dosya yolunu göster komutu
    context.subscriptions.push(vscode.commands.registerCommand('wcagEnhancer.showJsonPath', async () => {
        if (!jsonManager) {
            vscode.window.showErrorMessage("❌ JSON Manager bulunamadı");
            return;
        }
        const jsonPath = jsonManager.getJsonFilePath();
        const action = await vscode.window.showInformationMessage(`📁 AccessiMind JSON dosya yolu:\n${jsonPath}`, "Dosyayı Aç", "Klasörü Aç", "Yolu Kopyala");
        switch (action) {
            case "Dosyayı Aç":
    ...
    context.subscriptions.push(vscode.commands.registerCommand('wcagEnhancer.showJsonStatus', async () => {
        try {
            if (!jsonManager) {
                vscode.window.showErrorMessage("❌ JSON Manager bulunamadı");
                return;
...
🔧 Toplam İyileştirme: ${settings.statistics.totalImprovements}`;
            ...
            logger_1.logger.error("❌ JSON durumu gösterme hatası:", error);
            vscode.window.showErrorMessage("❌ JSON durumu alınamadı: " + error);
        }
    }));
    // JSON dosyasını sıfırla komutu
    context.subscriptions.push(vscode.commands.registerCommand('wcagEnhancer.resetJsonFile', async () => {
        const action = await vscode.window.showWarningMessage('⚠️ AccessiMind JSON dosyasını sıfırlamak istediğinizden emin misiniz?\n\nBu işlem tüm wizard ve ayar geçmişinizi silecektir.', { modal: true }, 'JSON Dosyasını Sıfırla');
        if (action === 'JSON Dosyasını Sıfırla') {
            try {
                ...
                jsonManager.dispose();
                const newJsonManager = accessiMindJsonManager_1.AccessiMindJsonManager.getInstance(jsonManager.context);
                ...
                logger_1.logger.error("❌ JSON sıfırlama hatası:", error);
                vscode.window.showErrorMessage("❌ JSON dosyası sıfırlanamadı: " + error);
            }
        }
    }));
    // JSON dosyası sağlık kontrolü komutu
    context.subscriptions.push(vscode.commands.registerCommand('wcagEnhancer.validateJsonHealth', async () => {
        try {
            if (!jsonManager) {
                vscode.window.showErrorMessage("❌ JSON Manager bulunamadı");
                return;
            ...
            if (health.isHealthy) {
                ...
                const action = await vscode.window.showWarningMessage(`⚠️ JSON dosyasında sorunlar bulundu:\n${health.issues.join('\n')}`, "Dosyayı Onar", "Detayları Göster");
            ...
            logger_1.logger.error("❌ JSON sağlık kontrolü hatası:", error);
            vscode.window.showErrorMessage("❌ Sağlık kontrolü başarısız: " + error);
        }
    }));
    // JSON dosyasını onar komutu
    context.subscriptions.push(vscode.commands.registerCommand('wcagEnhancer.repairJsonFile', async () => {
        try {
            if (!jsonManager) {
                vscode.window.showErrorMessage("❌ JSON Manager bulunamadı");
                return;
            }
            const action = await vscode.window.showWarningMessage('🔧 JSON dosyasını onarım işlemi:\n\n• Mevcut dosya yedeklenecek\n• Yeni default dosya oluşturulacak\n• Mevcut wizard durumu kaybolabilir\n\nDevam etmek istiyor musunuz?', { modal: true }, 'Dosyayı Onar');
            if (action === 'Dosyayı Onar') {
                await jsonManager.repairJsonFile();
            }
        }
        catch (error) {
            logger_1.logger.error("❌ JSON onarım hatası:", error);
            vscode.window.showErrorMessage("❌ Dosya onarılamadı: " + error);
        }
    }));
}
function deactivate() {
    logger_1.logger.info('🔄 AccessiMind deactivating...');
    ...
    logger_1.logger.info('✅ AccessiMind successfully deactivated');
}
