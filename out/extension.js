...
const modernSettingsPanel_1 = require("./views/modernSettingsPanel");
const helpPanel_1 = require("./views/helpPanel");
...
async function activate(context) {
    try {
        ...
        context.subscriptions.push(vscode.window.registerWebviewViewProvider('wcagEnhancer.statsView', statsViewProvider), vscode.window.registerWebviewViewProvider('wcagEnhancer.modernStatsView', modernStatsViewProvider), vscode.window.registerWebviewViewProvider('wcagEnhancer.tabbedMainView', tabbedMainViewProvider), vscode.window.registerWebviewViewProvider('wcagEnhancer.chatView', new chatViewProvider_1.ChatViewProvider(context.extensionUri)), vscode.window.registerTreeDataProvider('wcagEnhancer.settingsView', settingsViewProvider));
        // Register commands
        ...
        logger_1.logger.error('❌ AccessiMind activation failed:', error);
...
function registerCommands(context) {
    ...
    vscode.commands.registerCommand('wcagEnhancer.openSettings', async () => {
        modernSettingsPanel_1.ModernSettingsPanel.createOrShow(context);
    }), 
    // Open Help Panel
    vscode.commands.registerCommand('wcagEnhancer.openHelp', async () => {
        helpPanel_1.HelpPanel.createOrShow();
    }), 
...
async function handleInlineChat() {
    const editor = vscode.window.activeTextEditor;
    if (!editor) {
        vscode.window.showErrorMessage("❌ No active file found");
        return;
...
function getStatsWebViewContent(stats) {
    ...
    const dailyStats = stats.dailyStats?.[today] || { improvements: 0, linesImproved: 0, tokensUsed: 0 };
