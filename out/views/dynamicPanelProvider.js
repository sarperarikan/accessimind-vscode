...
class DynamicPanelProvider {
    constructor(context) {
        ...
        this._statisticsManager = statisticsManager_1.StatisticsManager.getInstance(context);
        this._aiProviderManager = aiProvider_1.AIProviderManager.getInstance();
    ...
    async showPanel() {
        ...
        this._panel = vscode.window.createWebviewPanel("wcagEnhancerDynamicPanel", "♿ AccessiMind - Dynamic Panel", vscode.ViewColumn.One, {
            enableScripts: true,
    ...
    setupRealTimeListeners() {
        ...
        logger_1.logger.info("DynamicPanelProvider real-time listeners initialized");
    }
    pushStatisticsUpdate(stats) {
    ...
    pushImprovementNotification(record) {
        if (this._panel && this._panel.visible) {
            this._panel.webview.postMessage({
    ...
    pushErrorNotification(errorRecord) {
        if (this._panel && this._panel.visible) {
            this._panel.webview.postMessage({
                type: "realTimeNotification",
    ...
    pushStatisticsResetNotification() {
        if (this._panel && this._panel.visible) {
            this._panel.webview.postMessage({
                type: "realTimeNotification",
    ...
    async _handleMessage(message) {
        logger_1.logger.info(`DynamicPanelProvider: Handling message type: ${message.type}`, { message });
        switch (message.type) {
            case "getStatistics":
    ...
    async _resetStatistics() {
        try {
            ...
            logger_1.logger.info("Statistics reset successfully via Dynamic Panel");
        }
        catch (error) {
            logger_1.logger.error("Failed to reset statistics:", error);
            this._panel?.webview.postMessage({
                type: "notification",
    ...
    async _exportStatistics(format = "json") {
        try {
            ...
            if (uri) {
                ...
                logger_1.logger.info(`Statistics exported successfully: ${uri.fsPath}, Format: ${format}, Size: ${fileSizeKB}KB`);
            }
            else {
            ...
            logger_1.logger.error("Export statistics failed:", error);
            this._panel?.webview.postMessage({
                type: "notification",
    ...
    async _loadInitialData() {
        try {
            logger_1.logger.info("DynamicPanelProvider: Loading initial data...");
            ...
            const currentProviderInstance = await this._aiProviderManager.getCurrentProviderInstance();
            const isAvailable = await currentProviderInstance.isAvailable();
            logger_1.logger.info(`DynamicPanelProvider: Initial data - Provider: ${currentProvider}, Available: ${isAvailable}, Display Name: ${currentProviderInstance.getDisplayName()}`);
            // Provider-specific availability checks
            if (currentProvider === "gemini") {
                const geminiProvider = currentProviderInstance; // Cast to access specific methods
                if (geminiProvider.isApiKeyConfigured) {
                    const isConfigured = await geminiProvider.isApiKeyConfigured();
                    logger_1.logger.info(`DynamicPanelProvider: Gemini API key configured: ${isConfigured}`);
                    if (!isConfigured) {
                        logger_1.logger.warn("DynamicPanelProvider: Gemini API key not configured");
                    }
                }
            }
            else if (currentProvider === "vscode-copilot") {
                const copilotProvider = currentProviderInstance;
                if (copilotProvider.getAvailableModels) {
                    const models = await copilotProvider.getAvailableModels();
                    logger_1.logger.info(`DynamicPanelProvider: VS Code Copilot models available: ${models.length}`, models);
                }
            }
            this._panel?.webview.postMessage({
                type: "initialData",
            ...
            logger_1.logger.info("DynamicPanelProvider: Initial data loaded successfully");
        }
        catch (error) {
            logger_1.logger.error("DynamicPanelProvider: Failed to load initial data:", error);
            this._panel?.webview.postMessage({
                type: "error",
    ...
    async _loadWizardSettings() {
        try {
            logger_1.logger.info("DynamicPanelProvider: Loading wizard settings...");
            ...
            logger_1.logger.info("DynamicPanelProvider: Wizard settings loaded:", wizardSettings);
            ...
            logger_1.logger.info("DynamicPanelProvider: Wizard settings sent to webview");
        }
        catch (error) {
            logger_1.logger.error("DynamicPanelProvider: Failed to load wizard settings:", error);
            this._panel?.webview.postMessage({
                type: "error",
