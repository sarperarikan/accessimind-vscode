...
class StatsViewProvider {
    ...
    setupRealTimeListeners() {
        ...
        logger_1.logger.info("StatsViewProvider real-time listeners initialized");
    }
    ...
    getFilteredStats(period, language) {
        ...
        if (startDate) {
            ...
            filteredImprovements = filteredImprovements.filter((rec) => rec.timestamp >= startTimestamp);
        }
        // Dile göre filtrele
        if (language && language !== "all") {
            filteredImprovements = filteredImprovements.filter((rec) => rec.language === language);
        }
        // Toplamlar ve detaylar
        const result = {
            totalImprovements: filteredImprovements.length,
            totalLinesImproved: filteredImprovements.reduce((sum, rec) => sum + (rec.linesImproved || 0), 0),
            totalTokensUsed: filteredImprovements.reduce((sum, rec) => sum + (rec.tokensUsed || 0), 0),
            totalProcessingTime: filteredImprovements.reduce((sum, rec) => sum + (rec.processingTime || 0), 0),
            dailyStats: {},
            languageStats: {},
            wcagCriteriaStats: {},
        ...
        // Günlük istatistikler
        filteredImprovements.forEach((rec) => {
    ...
    sendTokenAnalytics() {
        if (!this._statisticsManager || !this._view)
            return;
        try {
