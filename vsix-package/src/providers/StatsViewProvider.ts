...
export class StatsViewProvider implements vscode.WebviewViewProvider {
    ...
    private async resetStats(): Promise<void> {
        const result = await vscode.window.showWarningMessage(
            'İstatistikleri sıfırlamak istediğinizden emin misiniz? Bu işlem geri alınamaz.',
            'Sıfırla',
            'İptal'
        );
        ...
        if (result === 'Sıfırla') {
            ...
            await (this.settingsManager as any).context.globalState.update('wcagEnhancer.stats', defaultStats);
    ...
    private async exportStats(): Promise<void> {
        try {
            ...
            vscode.window.showInformationMessage(
                '📋 İstatistikler panoya kopyalandı',
                'Dosyaya Kaydet'
