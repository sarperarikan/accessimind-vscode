...
const AIServiceManager_1 = require("./services/AIServiceManager");
const WCAGDetectionService_1 = require("./services/WCAGDetectionService");
let settingsManager;
let aiServiceManager;
let wcagDetectionService;
function activate(context) {
    ...
    aiServiceManager = new AIServiceManager_1.AIServiceManager(settingsManager);
    wcagDetectionService = new WCAGDetectionService_1.WCAGDetectionService(settingsManager, aiServiceManager);
    // Initialize services
...
async function setupAutoDetection(context) {
    ...
    if (settings.autoDetectIssues) {
        ...
        if (workspaceFolders) {
            for (const folder of workspaceFolders) {
                const files = await vscode.workspace.findFiles(new vscode.RelativePattern(folder, '**/*.{html,htm,css,js,ts,jsx,tsx,vue,svelte}'), '**/node_modules/**', 10 // Limit initial scan
                );
...
function registerViewProviders(context) {
    ...
    context.subscriptions.push(vscode.window.registerWebviewViewProvider('accessimind.tabbedMainView', tabbedProvider));
    ...
    context.subscriptions.push(vscode.window.registerWebviewViewProvider('accessimind.dynamicPanelView', dynamicProvider));
    ...
    context.subscriptions.push(vscode.window.registerWebviewViewProvider('accessimind.statsView', statsProvider));
}
function registerCommands(context) {
    ...
    context.subscriptions.push(analyzeCommand, improveCommand, wizardCommand, autoDetectCommand, setDetailLevelCommand, endToEndImproveCommand, improveCurrentFileCommand, improveSelectedCodeCommand, createJiraTaskCommand, createJiraTaskFromSelectionCommand, showStatisticsCommand);
}
async function autoDetectIssues() {
    ...
    try {
        ...
        if (autoFixCount > 0) {
            ...
            const action = await vscode.window.showInformationMessage(`♿ ${completedMessage}`, fixText);
            if (action === fixText) {
...
async function setDetailLevel() {
    ...
    if (selected) {
        const newSettings = { ...settings, detailLevel: selected.value };
...
async function endToEndImprovement() {
    ...
    try {
        ...
        await vscode.window.withProgress({
            ...
            const action = await vscode.window.showInformationMessage(`♿ ${summaryMessage}`, applyText, showDetailsText);
            if (action === applyText) {
...
${analysisResult.suggestions.map(suggestion => `- ${suggestion.title}: ${suggestion.description}`).join('\n')}`;
        ...
        const jiraUrl = vscode.workspace.getConfiguration('accessimind').get('jiraUrl', '');
        const jiraProject = vscode.workspace.getConfiguration('accessimind').get('jiraProject', '');
        ...
        const action = await vscode.window.showInformationMessage(`♿ ${settings.language === 'en' ? 'Jira task content prepared!' : 'Jira görevi içeriği hazırlandı!'}`, openJiraText, copyText);
        if (action === copyText) {
...
async function improveCurrentCode() {
    ...
    try {
        ...
        const action = await vscode.window.showInformationMessage(`♿ ${settings.language === 'en' ? 'Code improvements ready!' : 'Kod iyileştirmeleri hazır!'}`, applyText, previewText);
        if (action === applyText) {
            // Apply improvements directly
            const edit = new vscode.WorkspaceEdit();
            const fullRange = new vscode.Range(editor.document.positionAt(0), editor.document.positionAt(code.length));
...
async function showSetupWizard(context) {
    ...
    const panel = vscode.window.createWebviewPanel('accessimind.wizard', isEnglish ? 'AccessiMind Setup Wizard' : 'AccessiMind Kurulum Sihirbazı', vscode.ViewColumn.One, {
        enableScripts: true,
