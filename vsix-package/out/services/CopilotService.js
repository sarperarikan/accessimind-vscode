...
class CopilotService extends AIService_1.AIService {
    ...
    async requestCopilotCompletion(prompt, language) {
        try {
            if (this.copilotAPI) {
    ...
    buildAccessibilityPrompt(code, language, action, detailLevel, issues) {
    ...
    parseAnalysisResult(result, detailLevel) {
    ...
    parseImprovementResult(originalCode, result) {
        return {
            originalCode,
    ...
    getBasicAccessibilitySuggestions(code, filePath) {
        const suggestions = [];
        const language = this.getLanguageFromFilePath(filePath);
        if (language === 'html') {
    ...
    applyBasicImprovements(code, filePath) {
        let improvedCode = code;
        const language = this.getLanguageFromFilePath(filePath);
        if (language === 'html') {
            // Add basic accessibility improvements
            improvedCode = improvedCode.replace(/<img([^>]*?)src="([^"]*)"([^>]*?)>/gi, (match, before, src, after) => {
            ...
            improvedCode = improvedCode.replace(/<button([^>]*?)>/gi, (match) => {
