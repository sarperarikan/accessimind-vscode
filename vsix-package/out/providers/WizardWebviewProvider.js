...
class WizardWebviewProvider {
    ...
    async testAIConnection(data) {
        try {
            ...
            const { AIServiceManager } = await Promise.resolve().then(() => require('../services/AIServiceManager'));
    ...
    async changeLanguage(language) {
        const currentSettings = await this.settingsManager.getAccessibilitySettings();
        const newSettings = { ...currentSettings, language: language };
