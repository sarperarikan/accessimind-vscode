...
export interface TokenUsage {
	...
	outputTokens: number;
	timestamp: number;
	operationType: "improvement" | "analysis" | "jira";
	codeLanguage: string;
...
export interface TokenPrice {
	model: string;
	provider: string;
	inputPricePerK: number; // USD per 1000 input tokens
	outputPricePerK: number; // USD per 1000 output tokens
	lastUpdated: number;
}
...
export interface TokenAnalytics {
	...
	costByLanguage: { [language: string]: number };
	efficiency: TokenEfficiencyMetrics;
	trends: {
		daily: { [date: string]: { cost: number; tokens: number } };
		weekly: { [week: string]: { cost: number; tokens: number } };
...
export class TokenAnalyticsManager {
	private static instance: TokenAnalyticsManager;
	...
	public static getInstance(context?: vscode.ExtensionContext): TokenAnalyticsManager {
	...
	public recordTokenUsage(usage: Omit<TokenUsage, "timestamp" | "costUSD">): void {
		try {
			...
			this.saveTokenData();
			logger.info(`Token usage recorded: ${usage.tokensUsed} tokens, $${cost.toFixed(4)} cost`);
		} catch (error) {
			logger.error("Error recording token usage:", error);
		}
	}

	public calculateTokenCost(model: string, inputTokens: number, outputTokens: number): number {
		...
		if (!pricing) {
			...
			if (!pricing) {
				logger.warn(`No pricing data for model: ${model}, using default`);
				return 0;
			}
		}
	...
	public estimateTokensFromCode(code: string, language: string): { input: number; output: number } {
	...
	public getTokenAnalytics(): TokenAnalytics {
		const now = Date.now();
		...
		const costByLanguage: { [language: string]: number } = {};
	...
	private calculateEfficiencyMetrics(usage: TokenUsage[]): TokenEfficiencyMetrics {
		if (usage.length === 0) {
			return {
				tokensPerLine: 0,
				tokensPerSecond: 0,
	...
	private generateTrends(usage: TokenUsage[]): TokenAnalytics["trends"] {
		const daily: { [date: string]: { cost: number; tokens: number } } = {};
		const weekly: { [week: string]: { cost: number; tokens: number } } = {};
		const monthly: { [month: string]: { cost: number; tokens: number } } = {};
		...
		usage.forEach(u => {
			...
			// Daily
			if (!daily[dateStr]) daily[dateStr] = { cost: 0, tokens: 0 };
			daily[dateStr].cost += cost;
			daily[dateStr].tokens += tokens;

			// Weekly
			if (!weekly[weekStr]) weekly[weekStr] = { cost: 0, tokens: 0 };
			weekly[weekStr].cost += cost;
			weekly[weekStr].tokens += tokens;

			// Monthly
			if (!monthly[monthStr]) monthly[monthStr] = { cost: 0, tokens: 0 };
	...
	private generateRecommendations(usage: TokenUsage[]): TokenAnalytics["recommendations"] {
		const modelEfficiency: { [model: string]: { cost: number; operations: number; efficiency: number } } = {};
	...
	public predictTokenUsage(code: string, language: string, operationType: "improvement" | "analysis" | "jira"): TokenPrediction {
		...
		const similarOperations = this.tokenUsageHistory.filter(u =>
	...
	public updateTokenPrices(newPrices: TokenPrice[]): void {
	...
	public getTokenPrice(model: string): TokenPrice | undefined {
		return this.tokenPrices.get(model);
	}

	public exportTokenData(): any {
		return {
	...
	private loadTokenData(): void {
		try {
			const data = this.context.globalState.get<any>("wcagEnhancer.tokenData", {});
