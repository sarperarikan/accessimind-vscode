export interface WcagImprovement {
	name: string
	description: string
	apply: (code: string) => string
}

export async function applyWcagImprovements(
	code: string,
	improvements: string[]
): Promise<string> {
...
function improveFormAccessibility(code: string): string {
	...
	if (improvedCode.includes("<form") && !improvedCode.includes("role=\"form\"")) {
		improvedCode = improvedCode.replace(
			/<form([^>]*?)>/g,
			"<form$1 role=\"form\" aria-labelledby=\"form-title\">"
		);
	}
...
function addAriaLabels(code: string): string {
	let improvedCode = code;
	...
	if (improvedCode.includes("<nav") && !improvedCode.includes("aria-label")) {
		improvedCode = improvedCode.replace(
			/<nav([^>]*?)>/g,
			"<nav$1 aria-label=\"Ana navigasyon\">"
		);
	}
...
function addKeyboardNavigation(code: string): string {
	...
	improvedCode = improvedCode.replace(
		/<button([^>]*?)>/g,
		(match, attributes) => {
			if (attributes.includes("tabindex")) return match;
...
function addColorContrast(code: string): string {
	let improvedCode = code;
...
<style>
	...
	if (!improvedCode.includes("<style>")) {
		...
		improvedCode = improvedCode.replace(
			/<\/style>/g,
			`${contrastStyles.replace("<style>", "").replace("</style>", "")}\n</style>`
		);
	}
...
function addImageAltText(code: string): string {
	let improvedCode = code;
...
function improveTableAccessibility(code: string): string {
	let improvedCode = code;
	...
	improvedCode = improvedCode.replace(
		/<table([^>]*?)>/g,
		(match, attributes) => {
			if (improvedCode.includes("<caption>")) return match;
...
function addErrorMessages(code: string): string {
	let improvedCode = code;
	...
	improvedCode = improvedCode.replace(
		...
		(match, attributes) => {
			...
			if (improvedCode.includes(`${id}-error`)) return match;
	...
	improvedCode = improvedCode.replace(
		...
		(match, beforeType, afterType, text) => {
