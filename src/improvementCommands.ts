			...
			if (improvementResult.success && improvementResult.improvedCode) {
				...
				if (autoApply) {
					...
					vscode.window.showInformationMessage(
						`✅ ${linesImproved} satır iyileştirildi ve otomatik olarak uygulandı! (${wcagCriteria.length} WCAG kriteri) - ${aiProviderManager.getCurrentProviderName()} (${aiProviderManager.getCurrentModelName()})`
					);
				} else {
					...
					await vscode.workspace.openTextDocument(improvedUri).then(async (_improvedDoc) => {
						...
						vscode.window.showInformationMessage(
							`✅ ${linesImproved} satır iyileştirildi! (${wcagCriteria.length} WCAG kriteri) - ${aiProviderManager.getCurrentProviderName()} (${aiProviderManager.getCurrentModelName()})`,
		...
		await vscode.window.withProgress({
			...
			if (improvementResult.success && improvementResult.improvedCode) {
				...
				if (autoApply) {
					...
					vscode.window.showInformationMessage(
						`✅ Seçili ${linesImproved} satır iyileştirildi ve otomatik olarak uygulandı! (${wcagCriteria.length} WCAG kriteri) - ${aiProviderManager.getCurrentProviderName()} (${aiProviderManager.getCurrentModelName()})`
					);
				} else {
					// Show improvement result and ask for confirmation
					vscode.window.showInformationMessage(
						`✅ Seçili kod iyileştirildi! ${linesImproved} satır, ${wcagCriteria.length} WCAG kriteri - ${aiProviderManager.getCurrentProviderName()} (${aiProviderManager.getCurrentModelName()})`,
						"Değişiklikleri Uygula",
						"Önizleme Göster",
						"İstatistikleri Görüntüle"
					).then(async action => {
