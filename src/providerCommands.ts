...
export async function improveCurrentFileWithProvider(providerType: "gemini" | "vscode-copilot"): Promise<void> {
	...
	try {
			...
			try {
				...
				if (improvementResult.success && improvementResult.improvedCode) {
					...
					if (autoApply) {
						...
						vscode.window.showInformationMessage(
							`✅ ${linesImproved} satır ${providerType === "gemini" ? "Gemini" : "Copilot"} ile iyileştirildi! (${wcagCriteria.length} WCAG kriteri)`
						);
					} else {
						...
						await vscode.workspace.openTextDocument(improvedUri).then(async (_improvedDoc) => {
							...
							await vscode.commands.executeCommand("vscode.diff", originalUri, improvedUri, 
								`${fileName} - ${providerType === "gemini" ? "Gemini" : "Copilot"} WCAG İyileştirmesi`);
					...
					vscode.window.showErrorMessage(
						`❌ ${providerType === "gemini" ? "Gemini" : "Copilot"} iyileştirmesi başarısız: ${improvementResult.error || "Bilinmeyen hata"}`
					);
				}
			} finally {
				...
				if (originalProvider !== providerType) {
					try {
						...
						logger.warn("Orijinal sağlayıcı geri yüklenemedi:", error);
					}
				}
			}
		});
		...
		logger.error(`${providerType} dosya iyileştirme hatası:`, error);
	}
}
...
export async function improveSelectedCodeWithProvider(providerType: "gemini" | "vscode-copilot"): Promise<void> {
	...
	try {
			...
			try {
				...
				if (improvementResult.success && improvementResult.improvedCode) {
					...
					if (autoApply) {
						...
						vscode.window.showInformationMessage(
							`✅ Seçili ${linesImproved} satır ${providerType === "gemini" ? "Gemini" : "Copilot"} ile iyileştirildi! (${wcagCriteria.length} WCAG kriteri)`
						);
					} else {
						// Show improvement result and ask for confirmation
						vscode.window.showInformationMessage(
							`✅ Seçili kod ${providerType === "gemini" ? "Gemini" : "Copilot"} ile iyileştirildi! ${linesImproved} satır, ${wcagCriteria.length} WCAG kriteri`,
							"Değişiklikleri Uygula",
							"Önizleme Göster",
							"İstatistikleri Görüntüle"
						).then(async action => {
							if (action === "Değişiklikleri Uygula") {
								...
								await vscode.commands.executeCommand("vscode.diff", originalUri, improvedUri, 
									`${providerType === "gemini" ? "Gemini" : "Copilot"} - Seçili Kod WCAG İyileştirmesi`);
							} else if (action === "İstatistikleri Görüntüle") {
								vscode.commands.executeCommand("wcagEnhancer.showDetailedStatistics");
							}
						});
					...
					vscode.window.showErrorMessage(
						`❌ ${providerType === "gemini" ? "Gemini" : "Copilot"} seçili kod iyileştirmesi başarısız: ${improvementResult.error || "Bilinmeyen hata"}`
					);
				}
			} finally {
				// Restore original provider if different
		...
		logger.error(`${providerType} seçili kod iyileştirme hatası:`, error);
	}
}
