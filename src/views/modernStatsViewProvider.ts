...
export class ModernStatsViewProvider implements vscode.WebviewViewProvider {
	public static readonly viewType = "wcagEnhancer.modernStatsView";
	...
	private setupRealTimeListeners(): void {
		...
		logger.info("ModernStatsViewProvider real-time listeners initialized");
	}
	...
	private updateStats() {
		if (!this._view || !this._statisticsManager) return;
	...
	private getFilteredStats(period: string, language?: string): DetailedStatistics {
		if (!this._statisticsManager) {
			logger.warn("StatisticsManager not available for filtering");
			return {
				totalImprovements: 0,
				totalLinesImproved: 0,
				totalTokensUsed: 0,
	...
	private async showResetMenu(): Promise<void> {
		if (!this._view) return;
	...
	private async resetPeriodStats(period: "daily" | "monthly" | "yearly"): Promise<void> {
		try {
			const config = vscode.workspace.getConfiguration("wcagEnhancer");
			const language = config.get("language", "en");
			const isEnglish = language === "en";
	...
	private async showDetailedStatsPanel(): Promise<void> {
